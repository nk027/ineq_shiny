...
data <- readRDS("data/eurostat.rds") # Break this down, atm only inc_mean & inc_med used
```
...
plot_inc <- renderPlotly({
  ...
  ggplot_inc <- ggplot(inc_mean_self, aes(x = time)) +
    ...
    geom_line(data = inc_mean_self, aes(y = values), colour = viris_full[1]) +
    geom_line(data = inc_med_self, aes(y = values), colour = viris_full[1], linetype = 3) +
...
plot_arpr <- renderPlotly({
  arpr %>% 
    ...
    layout(xaxis = list(title = "Year"),
           yaxis = list(title = "At-risk-of-poverty rate in %"))
})
