...
plot_quantiles <- renderPlotly({
  ...
  plot_ly(quant_st, x = ~time, y = ~QU1, name = "1st", type = "bar", color = I(teals[6]), 
          hoverinfo = "value") %>% 
    add_trace(y = ~QU2, name = "2nd", color = I(teals[4])) %>% 
    add_trace(y = ~QU3, name = "3rd", color = I(teals[3])) %>% 
    add_trace(y = ~QU4, name = "4th", color = I(teals[2])) %>% 
    add_trace(y = ~QU5, name = "5th", color = I(teals[1])) %>%  
    plotly::config(displaylogo = FALSE, collaborate = FALSE) %>% 
    plotly::layout(legend = list(orientation = "v", yanchor = "center", y = 0.6),
